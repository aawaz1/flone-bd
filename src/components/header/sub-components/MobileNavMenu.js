...
import SearchBox from "../../product/ShopSearch.js";
const MobileNavMenu = () => {
  ...
  return (
    <nav className="offcanvas-navigation" id="offcanvas-navigation">
      <ul>
        <li className="">
          <div> {userInfo && <h4> Hello {userInfo?.data?.auth?.email}</h4>}
          <SearchBox/>

          </div>
          
          <Link to={process.env.PUBLIC_URL + "/shop-grid-standard"}>{t("Products")}</Link>
        ...
        {/* <li className="menu-item-has-children">
         ...
         {/* <li>
          ...
          {userInfo ? (<> 
            ...
            {/* <Link to={process.env.PUBLIC_URL + "/"}>
            {t(userInfo.name.toUpperCase())}
              </Link> */}
             
        <li>
          <Link to={`/profile/${userInfo?.data?.auth?._id.toString()}`}>
            {t("profile")}
          </Link>
        </li>
          </>) : ("")}
        ...
        {userInfo ? (<li>
                  <Link onClick={logoutHandler} to={process.env.PUBLIC_URL + "/home-furniture-three"}>
                    {t("logout")}
                  </Link>
                </li>) : ""}
      </ul>
